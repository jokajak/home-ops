...
- **Human users** (`users.tf` + `users.sops.yaml`) — real people, non-superuser, defined as a
  **list** in a **SOPS-encrypted** YAML file committed to git, read via
  `yamldecode(data.sops_file.users.raw)["users"]`. Username is a field (not a map key), so every
...
- **`readers` group** — maps to **Grafana Viewer** (read) via the role_attribute_path; bound to
  the Grafana app for access. Add `readers` to a user's `groups` to grant Grafana read.
